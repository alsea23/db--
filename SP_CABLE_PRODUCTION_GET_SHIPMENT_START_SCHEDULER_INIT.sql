...
BEGIN
    ...
    -- 1. 전역 타임라인 기초 데이터 로드
    INSERT INTO #WORK_CNTR_TIMESTAMP (
          PROCESS_CODE
        , WORK_CNTR_CD
        , TS_IDX
        , IS_ASSIGNED
    ...
    SELECT 
          SALE_OPP_NO
        , PJT_SHIP
        , DATEDIFF(DAY, @TIMESTAMP_BASE_DATE, MIN(PRD_STRT_DATE))
    FROM #SRC_FJ
    GROUP BY 
          SALE_OPP_NO
        , PJT_SHIP;
    ...
    UPDATE T
    SET   T.IS_ASSIGNED = 1
        , T.SALE_OPP_NO = F.SALE_OPP_NO
        , T.LOT_NO      = F.LOT_NO
    FROM #WORK_CNTR_TIMESTAMP T
    JOIN @TB_FIXED_PJT_LOT_TIMESTAMP F 
      ON T.PROCESS_CODE = F.PROCESS_CODE 
    ...
    SELECT 
        ...
        , PROCESS_CODE
        , WORK_CNTR_CD
        , FIXED_TIMESTAMP_POINT
        , FIXED_TIMESTAMP_POINT + LEAD_TIME - 1
        , 1
        , -1
